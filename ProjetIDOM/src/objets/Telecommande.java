...
public class Telecommande extends ObjetNonRamassable {
	...
	public void verificationPiles() {
		...
		while (it.hasNext()) {
			...
			if (suivant.getNom().compareToIgnoreCase("Pile télécommande 1") == 0)
				indexP1 = index;
			else if (suivant.getNom().compareToIgnoreCase("Pile télécommande 2") == 0)
				indexP2 = index;
			else
				index++;
		}
		if (indexP1 != -1 && indexP2 != -1) {
			...
			Joueur.getInventaire().remove(indexP1);
			System.out.println("Vous utilisez la pile 1 de la télécommande");
			nombrePiles++;
		} else if (indexP2 != -1) {
			Joueur.getInventaire().remove(indexP2);
			System.out.println("Vous utilisez la pile 2 de la télécommande");
			nombrePiles++;
		}
	}
	...
	public void utiliserObjet() {
		...
		if (nombrePiles == 2) {
			...
			while (this.getUtilisation()) {
				...
				case "ETEINDRE TV":
					if (!allumee)
						...
						allumee = false;
						System.out.println("Vous avez éteint la télévision");
					}
					break;
				case "MUTE":
					if (!allumee)
						...
						if (sonActive) {
							System.out.println("Vous avez éteint le son");
							sonActive = false;
						} else {
							System.out.println("Vous avez allumé le son");
							sonActive = true;
						}
					}
					break;
			...
			System.out
					.println("Vous avez perdu les 2 piles de la télécommande... Retrouvez les pour pouvoir l'utiliser");
		}
	}
